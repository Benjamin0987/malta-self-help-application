...
export class AlbumService 
{
  ...
  /**
   * Gets the basic album data form photos
   */
  getAlbums(): Album[]
  {
    // map -> extracts the album names from the array
    // filter -> extracts unique items from the names
    const names = this.photos.map(photos => photos.album).filter((value, index, array) => array.indexOf(value) === index);

    // find => return the first array item that matches the condition
    // find => returns all items that match the condition
